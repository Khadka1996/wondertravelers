...
export default function TagPage() {
  ...
  const { user } = useAuth();
  const tag = Array.isArray(params?.tag) ? params.tag[0] : (params?.tag as string) || '';
  ...
  const fetchBlogsByTag = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetch(
        `${API_URL}/api/blogs/tag/${encodeURIComponent(tag)}?page=${currentPage}&limit=${blogsPerPage}`
      );
  ...
  }, [tag, currentPage, fetchBlogsByTag]);
  ...
  return (
    <>
      <main className="bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 min-h-screen">
        {/* Tag Header */}
        <div className="text-white py-8 px-4" style={{backgroundColor: '#0F172B'}}>
          <div className="max-w-6xl mx-auto">
            <h1 className="text-3xl font-bold mb-2">#{tag}</h1>
            <p className="text-blue-100 text-sm">Explore blogs tagged with <span className="font-semibold">{tag}</span></p>
          </div>
        </div>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-12">
          ...
          {!isLoading && blogs.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-slate-600 mb-8">No blogs found with tag <span className="font-semibold">#{tag}</span>.</p>
              <Link
                href="/blog"
